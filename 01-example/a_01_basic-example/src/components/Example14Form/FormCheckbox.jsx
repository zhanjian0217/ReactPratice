...
const FormCheckbox = () => {
  ...
  const onlyCheckListLabel = checkbox.filter((item) => item.checked).map((item) => item.label);
  ...
  return (
    <section data-name="FormCheckbox">
      ...
      {checkbox.map(({ label, checked }, index) => {
        return (
          <label key={label}>
            <input data-index={index} type="checkbox" value={label} checked={checked} onChange={atInputChange} />
            {label}
          </label>
        );
      })}
